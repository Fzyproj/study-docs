...
1. 面试遇到的架构设计问题，技术亮点。
   ...
   答：改造前，对系统挂起订单接口进行性能压测，发现当qps在1w的时候，导致接口频繁失败。由于这个接口是并非需要实时进行结果返回的，所以利用了异步的特性，使用mq的方式对原来老系统feign的调用方式进行改造，达到了解耦削峰的效果。

   技术亮点：

   1. 运用工厂模式将新接口和老接口进行兼容，通过阿波罗开关进行控制。
   2. mq系统解耦削峰
   3. 在挂起订单流程中，使用了线程池将非耦合任务进行批量操作，大大提升了接口的响应速度。

   **关键词**：挂起订单，压测，qps，feign，mq。
...
1. Runnable
2. Blocked: 等待锁资源。
3. Waiting：调用sleep方法，wait，join，park方法都能进入该状态。
4. Timed_Wating：调用sleep，以及上述其他放发并设置超时时间都可以。
5. dead：死亡。
...
​		当所有的对象都完成实例化和初始化操作之后，还要把完整对象放到容器中，此时在容器中存在对象的几个状态，完成实例化=但未完成初始化，完整状态，因为都在容器中，所以要使用不同的map结构来进行存储，此时就有了一级缓存和二级缓存，如果一级缓存中有了，那么二级缓存中就不会存在同名的对象，因为他们的查找顺序是1，2，3这样的方式来查找的。一级缓存中放的是完整对象，二级缓存中放的是非完整对象，即实例化好的对象，但是没有初始化好的对象。
